...
import { fetchJobRepliesWithID } from "@/utils/fetch/fetchJobReplies";
...
const SeekerScheduleCard: React.FC<Props> = ({ application }) => {
  ...
  useEffect(() => {
    const fillDatas = async () => {
      const replyData = await fetchJobRepliesWithID(String(application.reply));
      replyData && setReply(replyData);
      replyData && console.log(replyData);
    };
    fillDatas();
  }, []);
